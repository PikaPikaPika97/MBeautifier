    ...
    methods (Access = private)
        ...
        function state = createFormattingRunState(~, lineCount)
            state = struct( ...
                ...
                'containerDepth', 0, ...
                'continuousLines', {cell(max(1, lineCount), 3)}, ...
        ...
        function tf = shouldPreserveDeclarationContinuation(obj, contLineArray)
            tf = strcmpi(obj.Configuration.declarationSpacingStyle(), 'Preserve') ...
                ...
                && ~obj.shouldApplyInlineContinuousLineRules();
        end
        ...
        function [data, arrayMap] = replaceContainer(obj, data)
            formatElement = @(text, doIndexing, contType, isContainerElement) ...
                obj.performFormattingSingleLine(text, doIndexing, contType, isContainerElement);
            [data, arrayMap] = MBeautifier.ContainerFormatting.replaceContainers( ...
                data, obj.Configuration, obj.TokenStruct, obj.PreserveIndexExpressionSpacing, formatElement);
        end

        function data = restoreContainers(obj, data, map)
            data = MBeautifier.ContainerFormatting.restoreContainers(data, map, obj.Configuration);
        end
